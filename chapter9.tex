...
v_{c} &= V_{c}\sin \omega_{c}t+V_{c}\,m_{a}\sin \omega_{c}t\sin \omega_{m}t\label{eq9.15}
\end{align}
...
\begin{solution}
Let $I_{\carr}$ and $I_{t}$ be respectively the rms values of the unmoduled current and the total or modulated current. If $R$ is the resistance in which these currents flow, then we can write
\begin{align*}
...
$$
P_{t}-P_{c}=20-15.15=4.85\text{\,kW}
$$
The power in the LSB is
$$
P_{\text{LSB}}=\dfrac{4.85\,\text{kW}}{2}=2.425\text{\,kW}
$$
saving in power is $P_{t}-P_{\text{LSB}}=15.15\,\text{kW}-2.425\,\text{kW}=12.725\text{\,kW}$
$$
\% \ \text{Saving in Power} = \frac{P_{t}-P_{\text{LSB}}}{P_{t}}\times 100\%=\frac{12.725\,\text{kW}}{15.15\,\text{kW}}\times 100\%=84\%
$$
\vskip -1cm
\end{solution}
...
& T_{c}=\frac{1}{f_{c}}=\dfrac{\omega_{c}}{2\pi}\label{eq9.40}
\end{align}

Substituting Eqn.~\eqref{eq9.40} in Eqn.~\eqref{eq9.39}, we get
\begin{equation}
RC\gg \dfrac{\omega_{c}}{2\pi}\label{eq9.41}
\end{equation}
