...
Fig.~\ref{fig2.25} shows the circuit of negative series clipper\index{Clipping!negative series clipper} along with input and output waveforms. Note that the circuit operates with out $R_{L}$. Hence the load current $I_{L}$ is zero.
\begin{figure}[H]
\centering
...
\end{align*}
Choose, $I_{F}=1\mA$ [which is sufficient to keep the diode in on conduction]
$$
...
Fig.~\ref{fig2.34} shows the circuit of shunt noise clipper.\index{Shunt noise clipper} If consists of diodes connected in antiparallel between the output terminals. Shunt noise clipper is used to remove the noise riding on the peaks of an input waveform.
\begin{figure}[H]
\centering
